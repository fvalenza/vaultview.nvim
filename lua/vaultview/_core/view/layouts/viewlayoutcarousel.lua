...
function ViewLayoutCarousel:compute_layout()
    if not self.viewData or not self.viewData.pages or #self.viewData.pages == 0 then
        _LOGGER:warning("No pages found in viewData during layout computation")
        self:debug()
        return
    end
...
local space_taken_expanded = Constants.list_win[ViewLayoutCarousel.name()].width + 2

-- Collapsed list: width + 2 (padding + border)
local space_taken_collapsed = Constants.list_win_close[ViewLayoutCarousel.name()].width + 2
...
function ViewLayoutCarousel:compute_lists_in_page_visibility_window(page_idx)
    ...
    viewState.pages[page_idx].center_list_index = math.ceil((self.last_left_collapsed + self.last_right_collapsed) / 2)
end
...
---
--- @param page_idx integer Page index
--- @param list_idx integer List index
function ViewLayoutCarousel:compute_entries_in_list_visibility_window(page_idx, list_idx)
    -- TODO (roadmap):
